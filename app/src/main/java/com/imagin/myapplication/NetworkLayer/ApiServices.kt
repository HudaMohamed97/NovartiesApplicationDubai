package com.imagin.myapplication.NetworkLayer

import com.imagin.myapplication.Models.*
...
interface ApiServices {
    ...
    fun submitEventRate(
        @Path("rate_question") session: Int, @Body body: Map<String, String>, @Header(
            "Authorization"
        ) authHeader: String
    ): Call<SubmitModel>
    ...
    @POST("agenda-rating/{rate_question}")
    @FormUrlEncoded
    fun submitMutipleEventRate(
        @Path("rate_question") session: Int, @Field("options_ids[]") items: List<Int>, @Header(
            "Authorization"
        ) authHeader: String
    ): Call<SubmitModel>
